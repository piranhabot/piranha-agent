...
## 🔒 Advanced Security Hardening (v0.4.0)

### 6. Production-Grade Wasm Sandbox ✅

**Status:** Implemented via Wasmtime

All code generated by agents is executed in a strictly isolated **Wasmtime** sandbox.
- **Resource Capping**: Strict memory (10MB default) and CPU fuel limits.
- **Isolation**: No access to host file system, network, or environment variables.
- **Safe Execution**: Protects your machine from hallucination-driven logic errors.

### 7. Skill Permission Enforcement ✅

**Status:** Implemented via ContextVars

Skills verify agent authorization before execution.
- **Permission Tags**: Agents are assigned specific tags (e.g., `github_read`, `database_write`).
- **Context-Aware**: The skill itself checks the `agent_permissions` context variable.
- **Security**: Prevents unauthorized tool calls even if the LLM attempts them.

### 8. Egress Control & Secret Masking ✅

**Status:** Implemented

- **Domain Whitelisting**: Restrict outbound communication to specific hosts via `allowed_hosts`.
- **Secret Redaction**: The `SecretMasker` automatically scrubs API keys and passwords from logs and the Studio UI using advanced regex patterns.
- **Local-Only Binding**: The monitoring server defaults to `127.0.0.1` to prevent LAN exposure.
